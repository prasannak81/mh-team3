...
def camera_listener():
    ...
    Expects to receive JSON formatted data with the keys of:
      "state": [absent/present] to represent a vehicle in the parking spot
      "image": URL of the image to present to the Teams channel
    """
    if request.method == "POST":
        # Check for the state of the vehicle
        if request.json["state"] == "present":
            content = "Vehicle has arrived"
        elif request.json["state"] == "absent":
            content = "Vehicle has departed"
        ...
        api.messages.create(
            roomId=rooms[0], markdown=f"## {content}", files=[request.json["image"]]
        )
